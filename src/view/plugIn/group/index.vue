...
<div>
  <Form :model="listData.filter" inline :label-width="0" @submit.native.prevent="handleSearch" >
    ...
    <div class="fr">
      <FormItem label="插件名称" prop="groupId" :label-width="100">
      ...
      <FormItem label="插件类型" prop="groupId" :label-width="100">
        <Select v-model="listData.filter.groupId">
          <Option  :value="-1" :key="-1">全部</Option>
          <Option v-for="item in listData.groupList" :value="item.id" :key="item.id">{{ item.name }}</Option>
        </Select>
    ...
    <Table :loading="listData.loading" border stripe :columns="listData.columns" :data="listData.list" style="clear: both">
      <template slot="isBonus" slot-scope="{ column, row, index }">
        <i-switch size="large" v-model="listData.list[index].isBonus" @on-change="handleSetStatus(index)" :before-change="handleBeforeSetStatus" :true-value="constantData.stateType.ENABLED.value" :false-value="constantData.stateType.DISABLED.value">
  ...
  <Modal v-model="editModalData.visible" :width="640" :loading="editModalData.loading" :mask-closable="false" title="编辑素材" @on-ok="handleSubmit" @on-cancel="handleCancel" ok-text="保存">
    <Form ref="editModalForm" :model="editModalData.model" :rules="editModalData.rules" :label-width="120">
      ...
      <FormItem label="插件类型:" prop="groupId" >
        <Select v-model="listData.filter.groupId">
          <Option  :value="-1" :key="-1">全部</Option>
          <Option v-for="item in listData.groupList" :value="item.id" :key="item.id">{{ item.name }}</Option>
        </Select>
      </FormItem>
      <FormItem label="唯一标识:" prop="name">
        <Input v-model="editModalData.model.name" ></Input>
      </FormItem>
      <FormItem label="版本号:" prop="name">
        <Input v-model="editModalData.model.name" ></Input>
      </FormItem>
      <FormItem label="是否需要话术:" prop="groupId">
        <Select v-model="listData.filter.groupId">
          <Option  :value="-1" :key="-1">全部</Option>
          <Option v-for="item in listData.groupList" :value="item.id" :key="item.id">{{ item.name }}</Option>
        </Select>
      </FormItem>
      <FormItem label="是否需要Root：" prop="name">
        <Input v-model="editModalData.model.name" ></Input>
      </FormItem>
      <FormItem label="匹配版本：" prop="name">
        <Input v-model="editModalData.model.name" ></Input>
      </FormItem>
      <FormItem label="是否启用：" prop="name">
        <i-switch size="large" v-model="editModalData.model.appointment"  :true-value="1" :false-value="0">
          <span slot="open">是</span>
          <span slot="close">否</span>
        </i-switch>
      </FormItem>
      <FormItem prop="remark" label="插件的描述:" >
